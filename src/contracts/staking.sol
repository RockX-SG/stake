...
contract Staking is Initializable, PausableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // UPDATE: pectra feature contract
    ...
    */
    function initializeV3(address stakingV2_) public reinitializer(3) {
        stakingV2 = stakingV2_;
    }
        ...
        onlyRole(REGISTRY_ROLE)
    {
        ...
        for (uint256 i = 0; i < n; i++) {
            ...
            validatorRegistry.push(
                ValidatorCredential({
                    pubkey: pubkeys[i], signature: signatures[i], stopped: false, restaking: false, eigenpod: 0
                })
            );
        ...
        onlyRole(REGISTRY_ROLE)
    {
        ...
        for (uint256 i = 0; i < n; i++) {
            ...
            validatorRegistry.push(
                ValidatorCredential({
                    pubkey: pubkeys[i], signature: signatures[i], stopped: false, restaking: true, eigenpod: 0
                })
            );
            pubkeyIndices[pubkeyHash] = validatorRegistry.length;
        }
        ...
        onlyRole(REGISTRY_ROLE)
    {
        ...
        for (uint256 i = 0; i < n; i++) {
            ...
            validatorRegistry.push(
                ValidatorCredential({
                    pubkey: pubkeys[i], signature: signatures[i], stopped: false, restaking: true, eigenpod: podIds[i]
                })
            );
            pubkeyIndices[pubkeyHash] = validatorRegistry.length;
        }
    ...
    function currentReserve() public view returns (uint256) {
        return currentReserveV1() + IStaking(stakingV2).currentReserveV2();
    }
